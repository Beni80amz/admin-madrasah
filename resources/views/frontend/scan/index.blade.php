...
<head>
    ...
    <style>
        ...
        .scanner-frame-bottom::before, .scanner-frame-bottom::after {
            content: '';
            position: absolute;
            width: 40px;
            height: 40px;
        ...
        video { object-fit: cover; border-radius: 1.5rem; }
    </style>
</head>
<body class="bg-background-light dark:bg-background-dark text-text-main dark:text-gray-100 min-h-screen flex flex-col font-display overflow-x-hidden selection:bg-primary/30" x-data="scanApp()">
    
    <!-- 1. Header -->
    <header class="w-full pt-6 pb-2 px-6 flex items-center justify-between">
        <a href="{{ route('dashboard.index') }}" class="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
    ...
    <main class="flex-1 w-full max-w-md mx-auto px-6 flex flex-col items-center gap-6 pb-24">
        ...
        <div class="text-center mt-4">
            <div class="text-5xl font-black tracking-tight text-text-main dark:text-white flex items-baseline justify-center gap-1">
        ...
        <!-- 4. Scanner Area -->
        <div class="w-full aspect-square bg-black rounded-[2rem] relative scanner-frame shadow-2xl overflow-hidden group">
            ...
            <div class="absolute inset-0 z-10 flex items-center justify-center">
                ...
                <!-- Selfie Mode -->
                <video x-show="mode === 'selfie'" id="selfie-video" autoplay playsinline class="w-full h-full object-cover transform -scale-x-100"></video>
                <canvas id="selfie-canvas" class="hidden"></canvas>
                
                <!-- Captured Image Preview -->
                <img x-show="capturedImage" :src="capturedImage" class="absolute inset-0 w-full h-full object-cover z-20 transform -scale-x-100">
            </div>
            
            <div class="scanner-frame-bottom"></div>

            <!-- Overlay Text/Icon -->
            <div x-show="!capturedImage" class="absolute inset-0 z-30 flex flex-col items-center justify-center pointer-events-none">
                <!-- Toggle Mode Small Pills -->
                <div class="pointer-events-auto absolute top-6 bg-black/40 backdrop-blur-md rounded-full p-1 flex border border-white/10">
                     <button @click="setMode('qr')" class="px-3 py-1 rounded-full text-xs font-bold transition-colors" :class="mode === 'qr' ? 'bg-white text-black' : 'text-white/60 hover:text-white'">QR</button>
                     <button @click="setMode('selfie')" class="px-3 py-1 rounded-full text-xs font-bold transition-colors" :class="mode === 'selfie' ? 'bg-white text-black' : 'text-white/60 hover:text-white'">Selfie</button>
                </div>

                <div class="mt-8 bg-black/60 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/10 flex flex-col items-center text-center">
                    <span class="material-symbols-outlined text-white text-3xl mb-1" x-text="mode === 'qr' ? 'qr_code_Scanner' : 'face'"></span>
                    <p class="text-white text-xs font-medium" x-text="mode === 'qr' ? 'Arahkan ke QR Code' : 'Ambil Foto Selfie'"></p>
                </div>
            </div>

            <!-- Selfie Trigger Action -->
            <div x-show="mode === 'selfie' && !capturedImage" class="absolute bottom-6 w-full flex justify-center z-40 pointer-events-auto">
                 <button @click="takePhoto()" class="size-14 rounded-full border-4 border-white flex items-center justify-center bg-white/20 backdrop-blur-sm hover:bg-white/40 active:scale-95 transition-all">
                    <div class="size-10 bg-white rounded-full"></div>
                </button>
            </div>
            
             <!-- Retake Button -->
            <button x-show="capturedImage" @click="retakePhoto()" class="absolute top-4 right-4 z-50 p-2 rounded-full bg-black/50 text-white hover:bg-black/80 backdrop-blur-sm pointer-events-auto">
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white dark:bg-surface-dark border-t border-gray-100 dark:border-gray-800 p-4 z-50 rounded-t-[2rem] shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <div class="max-w-md mx-auto grid grid-cols-2 gap-4">
            ...
            <button @click="setAction('masuk')" 
                ...
                <div class="size-10 rounded-full bg-black/5 flex items-center justify-center mb-1">
                   <span class="material-symbols-outlined font-bold transform rotate-180">logout</span> <!-- Login icon usually arrow pointing in, logout arrow pointing out. 'login' icon exists. 'logout' with rotate looks like 'in' -->
                </div>
    ...
    <div x-show="alert.show" 
        ...
        <div class="max-w-md mx-auto bg-surface-light dark:bg-surface-dark border border-gray-100 dark:border-gray-800 shadow-2xl rounded-2xl p-4 flex items-center gap-4 pointer-events-auto">
            <div class="size-12 rounded-full flex items-center justify-center shrink-0"
                ...
                <span class="material-symbols-outlined" x-text="alert.type === 'success' ? 'check_circle' : 'error'"></span>
            </div>
            <div>
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.watchPosition(
                            (position) => {
                                ...
                                // Simple Mock Validation (Replace with real geofence distance check if needed client side)
                                // For now, if accuracy < 100m consider "In Area" visually, backend does real check
                                this.locationValid = true; 
                                this.locationStatus = 'Di Dalam Area Kantor'; 
                            },
                            (error) => {
                                this.locationValid = false;
                                this.locationStatus = 'Gagal Deteksi Lokasi';
                                this.accuracy = null;
                            },
                            { enableHighAccuracy: true, timeout: 20000, maximumAge: 0 }
                        );
                    } else {
                        this.locationStatus = 'GPS tidak didukung';
                    }
                },
                    ...
                    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                        navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" } })
                            ...
                            .catch((err) => {
                                ...
                                this.showAlert('error', 'Kamera Error', 'Izin kamera diperlukan.');
                            });
                    }
                },
