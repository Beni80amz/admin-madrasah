    ...
    <div class="max-w-[1200px] mx-auto w-full flex flex-col gap-12">
        ...
        <div class="flex flex-col gap-4">
            <div class="flex items-center gap-2 text-sm text-text-secondary-light dark:text-text-secondary-dark">
                ...
                <span class="material-symbols-outlined text-xs">chevron_right</span>
                <span class="text-text-primary-light dark:text-text-primary-dark">Prestasi Siswa</span>
            </div>
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex flex-col gap-3">
                    ...
                    <p
                        ...
                        Daftar pencapaian dan prestasi membanggakan dari para siswa
                        {{ $siteProfile->nama_madrasah ?? 'Madrasah' }} dalam berbagai bidang
                        kompetisi
                    </p>
                </div>
                @if($total > 0)
                    <a href="{{ route('akademik.prestasi-siswa.download') }}"
                    ...
                    <div class="p-6 pt-4 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            @if($achievement->photo)
                                <img src="{{ Storage::url($achievement->photo) }}" alt="{{ $achievement->nama }}"
                            ...
                            <div>
                                <p class="text-text-primary-light dark:text-text-primary-dark text-sm font-medium">
                                    {{ $achievement->nama }}</p>
                                <p class="text-gray-500 text-xs">{{ $achievement->kelas ?? '-' }}</p>
                            </div>
                        </div>
                        <span
            ...
            @empty
                <div class="col-span-full flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 class="text-text-primary-light dark:text-text-primary-dark text-xl font-bold mb-2">Belum Ada Data
                        Prestasi</h3>
                    <p class="text-text-secondary-light dark:text-text-secondary-dark">Data prestasi siswa akan ditampilkan
                        di sini</p>
                </div>
            @endforelse
        </div>
        ...
        <div
            ...
            <div class="flex flex-col gap-2">
                <h3 class="text-text-primary-light dark:text-text-primary-dark font-bold text-xl">Ingin Anak Anda
                    Berprestasi?</h3>
                <p class="text-text-secondary-light dark:text-text-secondary-dark text-sm max-w-lg">
                    Bergabunglah bersama kami dan kembangkan potensi terbaik putra-putri Anda di
